...
# move to a more predictible name
        mv /var/www/kanboard-$KANBOARD_VERSION /var/www/kanboard ;\
...
RUN     cp /var/www/kanboard/config.default.php /var/www/kanboard/config.php; \
        sed -i "s/define('DB_DRIVER', 'sqlite')/define('DB_DRIVER', 'mysql')/g" /var/www/kanboard/config.php; \
        sed -i "s/define('DB_USERNAME', 'root')/define('DB_USERNAME', '$KANBOARD_DB_USERNAME')/g" /var/www/kanboard/config.php; \
        sed -i "s/define('DB_PASSWORD', '')/define('DB_PASSWORD', '$KANBOARD_DB_PASSWD')/g" /var/www/kanboard/config.php; \
        sed -i "s/define('DB_HOSTNAME', 'localhost')/define('DB_HOSTNAME', '$KANBOARD_DB_HOST')/g" /var/www/kanboard/config.php;
